...
def router(request: str, parser_args: Namespace) -> bytes:
    ...
    return mapping.get(endpoint, View.not_found)(url=url, request_lines=request_lines, parser_args=parser_args)


def process_request(client_sock: socket.socket, client_addr: Tuple[str], parser_args: Namespace):
    """
...
def main():
    ...
    parser.add_argument("--directory", type=str, help="Directory to serve files from", default="/tmp/")
    parser_args = parser.parse_args()
    server_socket = socket.create_server(("localhost", 4221), reuse_port=True)
    server_socket.listen(5)
    while server_socket:
        client_sock, client_addr = server_socket.accept()  # wait for client
        Thread(target=process_request, args=(client_sock, client_addr, parser_args)).start()
